...
import { apiFetchTasks, apiCreateTask, apiUpdateTask, apiDeleteTask } from "./src/api/client.js";
...
async function toggleTaskCompletion(taskId) {
  ...
  try {
    await apiUpdateTask(taskId, { completed: newCompleted });
    ...
    task.completed = !newCompleted;
    task.completedAt = newCompleted ? null : new Date().toISOString();
    showNetworkError(err.message);
  }
  refresh();
}
...
async function completeAllTasks() {
  ...
  try {
    await Promise.all(pending.map(t => apiUpdateTask(t.id, { completed: true })));
    pending.forEach(() => { days[key] = (days[key] || 0) + 1; });
    persistCompletedDays(days);
  ...
  async function save() {
    ...
    if (newText && newText !== original) {
      ...
      if (t) {
        t.text = newText;
        try {
          await apiUpdateTask(task.id, { text: newText });
        } catch (err) {
          t.text = original;
          showNetworkError(err.message);
        }
