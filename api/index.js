...
app.put('/api/v1/tasks/:id', (req, res) => {
  ...
  if (completed !== undefined) {
    ...
    if (completed && !wasCompleted) tarea.completedAt = new Date().toISOString();
    else if (!completed && wasCompleted) tarea.completedAt = null;
  }
  res.json(tarea);
});
...
app.delete('/api/v1/tasks/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ error: 'El recurso solicitado no existe.' });
